 ...
 */
public final class OriginalNDS {
    /**
    ...
    private static final class SorterXD extends Sorter {
        ...
        private void sortHighByLow2D(int lFrom, int lUntil, int hFrom, int hUntil) {
            fenwickInit(indices, lFrom, lUntil);
            int li = lFrom;
        ...
        private void sortHighByLow(int lFrom, int lUntil, int hFrom, int hUntil, int dimension) {
            ...
            if (lSize == 1) {
                for (int hi = hFrom; hi < hUntil; ++hi) {
                    if (dominatesEq(indices[lFrom], indices[hi], dimension)) {
                        updateFront(indices[hi], indices[lFrom]);
                    }
                }
            } else if (hSize == 1) {
                for (int li = lFrom; li < lUntil; ++li) {
                    if (dominatesEq(indices[li], indices[hFrom], dimension)) {
                ...
                if (maxValue(lFrom, lUntil, dimension) <= minValue(hFrom, hUntil, dimension)) {
                    ...
                    System.arraycopy(indices, lFrom, indL, 0, lSize);
                    System.arraycopy(indices, hFrom, indH, 0, hSize);
                    ...
                    split3(indices, lFrom, lUntil, dimension, median);
                    int lMidL = lFrom + lessThan, lMidR = lMidL + equalTo;

                    split3(indices, hFrom, hUntil, dimension, median);
                    ...
                    System.arraycopy(indL, 0, indices, lFrom, lSize);
                    System.arraycopy(indH, 0, indices, hFrom, hSize);
                }
            }
        }
