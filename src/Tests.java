...
public class Tests {
    static int[] findFrontIndices(double[][] input) {
        ...
        int[] rv = new int[size];
        ParallelNDS.getSorter(size, dim).sort(input, rv);
//        OriginalNDS.getSorter(size, dim).sort(input, rv);
        return rv;
    }
    ...
    static void checkEqual(int[] expected, int[] found) {
        ...
        for (int i = 0; i < expected.length; ++i) {
            if (expected[i] != found[i]) {
                failed = true;
                System.out.print(i + ": (" + expected[i] + ", " + found[i] + "), ");
            }
        }
        if (failed)
