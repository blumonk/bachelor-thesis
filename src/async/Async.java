...
public final class Async {
    ...
    public static Sorter getSorter(int size, int dim, int threshold, int ndathresh) {
        if (dim < 0 || size < 0) {
            throw new IllegalArgumentException("Size or dimension is negative");
        }
        return new SorterXD(size, dim, threshold, ndathresh);
    }
    ...
    private static final class SorterXD extends Sorter {
        ...
        private final int NDATHRESHOLD;
        private static final ForkJoinPool pool = new ForkJoinPool(N_THREADS, ForkJoinPool.defaultForkJoinWorkerThreadFactory, null, true);
        private static final List<Context> emptyContext = new ArrayList<>();
        ...
        public SorterXD(int size, int dim, int threshold, int nda) {
            super(size, dim);
            this.threshold = threshold;
            this.NDATHRESHOLD = nda;
        ...
        protected void sortImpl(double[][] input, int[] output) {
            ...
            sort(0, size, dim - 1, emptyContext);
            for (int i = 0; i < size; ++i)
        ...
        private class Context {
            ...
            public Context(int[] hPart, int k, List<Future> futures) {
                this.k = k;
                this.hPart = hPart;
        ...
        private void syncSort(int from, int until, int dimension) {
            int size = until - from;
            if (size == 2) {
                ...
                if (dimension == 1) {
                    ...
                    if (allValuesEqual(from, until, dimension)) {
                        syncSort(from, until, dimension - 1);
                    } else {
                        ...
                        syncSort(midL, midH, dimension - 1);
        ...
        // NDHelperA
        private void sort(int from, int until, int dimension, List<Context> contexts) {
            int size = until - from;
            boolean hasContexts = contexts.size() > 0;
            ...
            if (size == 2) {
                ...
                if (hasContexts) {
                    int[] lIdx = new int[size];
                    System.arraycopy(indices, from, lIdx, 0, size);
                    for (int i = contexts.size() - 1; i >= 1; --i) {
                        Context c = contexts.get(i);
                        c.futures.add(pool.submit(new SortHighByLow(lIdx.clone(), c.hPart.clone(), c.k)));
                    }
                    Context c = contexts.get(0);
                    c.futures.add(pool.submit(new SortHighByLow(lIdx, c.hPart, c.k)));
                }
            } else if (size > 2) {
                if (dimension == 1) {
                    sort2D(from, until);
                    ...
                    if (allValuesEqual(from, until, dimension)) {
                        sort(from, until, dimension - 1, contexts);
                    } else {
                        System.arraycopy(indices, from, swap.get(), from, size);
                        double median = medianInSwap(from, until, dimension);
                        split3(indices, from, until, dimension, median);
                        int midL = from + lessThan.get(), midH = midL + equalTo.get();
                        boolean hasHWait = false, scheduledLH = false;
                        List<Future> hWait = null;
                        final int lSize = midL - from, mSize = midH - midL, hSize = until - midH;

                        if (lSize < NDATHRESHOLD) {
                            // NDA(L)
                            syncSort(from, midL, dimension);
                        } else {
                            // NDA(L) + async(NDB(L, H))
                            scheduledLH = true;
                            List<Context> newContexts = new ArrayList<>(contexts);
                            if (hSize > 0) {
                                int[] hIdx = new int[hSize];
                                System.arraycopy(indices, midH, hIdx, 0, hSize);
                                hWait = new ArrayList<>();
                                hasHWait = true;
                                Context ctx = new Context(hIdx, dimension - 1, hWait);
                                newContexts.add(ctx);
                            }
                            sort(from, midL, dimension, newContexts);
                        }

                        // NDB(L, M); mSize always < NDB threshold
                        if (mSize == 1) {
                        ...
                        // Process contexts with L
                        if (hasContexts && !scheduledLH && lSize > 0) {
                            int[] lIdx = new int[lSize];
                            System.arraycopy(indices, from, lIdx, 0, lSize);
                            for (int i = contexts.size() - 1; i >= 1; --i) {
                                Context c = contexts.get(i);
                                c.futures.add(pool.submit(new SortHighByLow(lIdx.clone(), c.hPart.clone(), c.k)));
                            }
                        ...
                        if (hasContexts) {
                            int[] mIdx = new int[mSize];
                            System.arraycopy(indices, midL, mIdx, 0, mSize);
                            for (int i = contexts.size() - 1; i >= 1; --i) {
                                Context c = contexts.get(i);
                                c.futures.add(pool.submit(new SortHighByLow(mIdx.clone(), c.hPart.clone(), c.k)));
                            }
                            Context c = contexts.get(0);
                            c.futures.add(pool.submit(new SortHighByLow(mIdx, c.hPart, c.k)));
                        }
                        ...
                        if (hasHWait) {
                            ...
                            System.arraycopy(indices, midL, mIdx, 0, mSize);
                            System.arraycopy(indices, midH, hIdx, 0, hSize);
                            pool.invoke(new SortHighByLow(mIdx, hIdx, dimension - 1));
                            merge(indices, from, midL, midH);
                        } else {
                            // NDB(L+M, H)
                            merge(indices, from, midL, midH);
                            int lmSize = midH - from;
                            if (lSize < threshold || hSize < threshold || dimension == 2) {
                                sortHighByLow(indices, from, midH, midH, until, dimension - 1);
                            } else {
                                int[] lIdx = new int[lmSize];
                                int[] hIdx = new int[hSize];
                                System.arraycopy(indices, from, lIdx, 0, lmSize);
                                System.arraycopy(indices, midH, hIdx, 0, hSize);
                                pool.invoke(new SortHighByLow(lIdx, hIdx, dimension - 1));
                            }
                        }
                        ...
                        if (hasHWait)
                            waitFutures(hWait);
                        boolean divideH = false;
                        if (hSize >= NDATHRESHOLD * 2) {
                            divideH = true;
                            sort(midH, until, dimension, contexts);
                        } else {
                            syncSort(midH, until, dimension);
                        }

                        if (hasContexts && !divideH && hSize > 0) {
                            int[] hIdx = new int[hSize];
                            System.arraycopy(indices, midH, hIdx, 0, hSize);
                            for (int i = contexts.size() - 1; i >= 1; --i) {
                                Context c = contexts.get(i);
                                c.futures.add(pool.submit(new SortHighByLow(hIdx.clone(), c.hPart.clone(), c.k)));
                            }
                            Context c = contexts.get(0);
                            c.futures.add(pool.submit(new SortHighByLow(hIdx, c.hPart, c.k)));
